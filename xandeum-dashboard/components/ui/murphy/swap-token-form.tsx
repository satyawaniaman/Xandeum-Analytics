...
interface QuoteResult {
  ...
  routeInfo: unknown;          // Route information
}
...
const customResolver = (data: SwapFormValues) => {
  ...
  // Validate amount
  if (data.amountIn === undefined || data.amountIn === null) {
    errors.amountIn = {
      type: "required",
...
export function SwapForm({
  // onSwap unused but kept for API compatibility
  tokens,
  isLoading = false,
  ...
  const fetchTokenAccounts = async (ownerPublicKey: PublicKey) => {
    try {
      ...
      // Extended default tokens list - XAND is the primary token
      const defaultTokens: TokenInfo[] = [
        {
          ...
          icon: "/crypto-logos/xandeum-logo.png",
        },
        {
          id: "sol",
          symbol: "SOL",
      ...
      try {
        ...
        for (const account of tokenAccounts.value) {
          ...
          if (tokenAmount.uiAmount > 0) {
            // Check if this token is already in defaultTokens
            const existingDefaultTokenIndex = defaultTokens.findIndex(
              t => t.mintAddress === mintAddress
            );

            if (existingDefaultTokenIndex >= 0) {
              // Update balance of existing default token
              defaultTokens[existingDefaultTokenIndex].balance = tokenAmount.uiAmount;
            } else {
              // Add as new token
              splTokens.push({
                id: mintAddress,
  ...
  const handleClear = (e?: React.MouseEvent) => {
    ...
    setAmountInValue("");
    setAmountOutValue("");
    form.setValue("amountIn", undefined, { shouldValidate: false });
    form.setValue("amountOut", undefined, { shouldValidate: false });

    // Clear tokens
    setSelectedTokenIn(null);
    setSelectedTokenOut(null);
    form.setValue("tokenIn", "", { shouldValidate: false });
    form.setValue("tokenOut", "", { shouldValidate: false });
  ...
  const handleUseMax = (e?: React.MouseEvent) => {
    ...
    if (selectedTokenIn && selectedTokenIn.balance > 0) {
      ...
      if (selectedTokenIn.id === "sol" ||
        ...
        // Keep 0.01 SOL for transaction fees, enough for most transactions
        maxAmount = Math.max(selectedTokenIn.balance - 0.01, 0);
      } else {
  ...
  const handleTokenChange = (isInput: boolean, tokenId: string) => {
    ...
    if (amountInValue && parseFloat(amountInValue) > 0) {
      if (isInput) {
        fetchSwapQuote(parseFloat(amountInValue), false, token, selectedTokenOut);
      } else {
        fetchSwapQuote(parseFloat(amountInValue), false, selectedTokenIn, token);
      }
    }
  };
  ...
  const fetchSwapQuote = async (
    ...
    overrideTokenIn?: TokenInfo | null,
    overrideTokenOut?: TokenInfo | null
  ) => {
    const tokenIn = overrideTokenIn !== undefined ? overrideTokenIn : selectedTokenIn;
    const tokenOut = overrideTokenOut !== undefined ? overrideTokenOut : selectedTokenOut;
    ...
    if (!tokenIn || !tokenOut) {
      // Don't show error toast here as user might be just selecting the first token
      return;
    }
    ...
    try {
      ...
      // Check token address
      if (!tokenIn.mintAddress || !tokenOut.mintAddress) {
        throw new Error("Token mintAddress not found");
      }

      // Create PublicKey from address
      const inputMint = new PublicKey(tokenIn.mintAddress);
      const outputMint = new PublicKey(tokenOut.mintAddress);
      const slippageBps = Math.floor(slippageValue * 100);

      // Call API to get quote
      // amount is passed as number (float) because the hook handles decimal conversion
  ...
  const onSubmit = async (values: SwapFormValues) => {
    ...
    try {
      ...
      try {
        ...
        // Pass fewer parameters, compatible with new API
        const result: any = await executeTrade(
          outputMint,
          values.amountIn,
          inputMint,
          slippageBps
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className="space-y-2">
                      ...
                      {selectedTokenIn && showTokenBalance && (
                        <div className="flex items-center text-xs text-muted-foreground space-x-1">
                          ...
                          <div className="max-button-container" onClick={(e) => e.stopPropagation()}>
                            ...
                            <span
                              className="cursor-pointer h-auto py-0 px-2 text-xs text-muted-foreground hover:text-primary hover:underline"
                              onClick={handleClear}
                            >
                              CLEAR
                            </span>
              ...
              <div className="flex justify-center -my-2 relative z-10 py-8">
                <Button
                  variant="outline"
                  size="icon"
